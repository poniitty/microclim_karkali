...
maxd <- as.Date("2022-11-18", tz = "Etc/GMT-2")

raw_data_dir <- "E:/karkali2022"
...
readdata <- function(i){
  ...
  if(nn > 1){
    ...
    diff <- d %>% 
      mutate(lagg = V2 - lag(V2)) %>% 
      pull(lagg) %>% max(., na.rm = T)
    
    if(diff > 1000){
      
      ff <- list.files(data_dir_old, pattern = as.character(fi$tomst_id[fi$file2 == i]), recursive = T, full.names = T)
      ...
      if(length(ff) == 1){
        ...
        d2 %>% select(V2,V3,V4,V5,V6,V7) -> d2
        
        d2 %>% filter(!duplicated(.$V2, fromLast = T)) -> d2
        
        d2 %>% mutate(across(V4:V6, ~as.numeric(gsub(",",".\\",.)))) -> d2
        
        d2$site <- fi[which(fi$file2 == i),"site"]
        
        d2 %>% mutate(V2 = ymd_hm(V2, tz = "UTC")) %>% 
          mutate(V2 = with_tz(V2, tzone = "Etc/GMT-2")) -> d2
        
        d <- bind_rows(d, d2) %>% 
          distinct()
        
        if(any(duplicated(d$V2))){
          print("WTF??!?!?")
        }
...
probls <- c(as_date(as_date("2019-07-03"):as_date("2019-09-23")),
            ...
            as_date(as_date("2022-08-27"):as_date("2022-11-18")))
hattu <- c()
...
office <- c(as_date(as_date(min(df$datetime)):as_date("2019-06-20")))
probls <- c()
hattu <- c(as_date(as_date("2022-04-24"):as_date("2022-11-18")))
...
office <- c(as_date(as_date(min(df$datetime)):as_date("2019-06-22")))
probls <- c(as_date(as_date("2022-03-22"):as_date("2022-05-23")))
hattu <- c()
moiste <- c(as_date(as_date("2022-03-20"):as_date("2022-07-20")),
            as_date(as_date("2022-07-28"):as_date("2022-07-30")))
